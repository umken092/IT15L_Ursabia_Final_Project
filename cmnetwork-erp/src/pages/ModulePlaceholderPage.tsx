...
const renderStatValue = (stat: ModuleStat) => {
  ...
  return <strong className="metric">{stat.value}</strong>
}
...
export const ModulePlaceholderPage = () => {
  ...
  if (moduleKey === 'user-management') {
    const allowedRoles = moduleRoleMap['user-management'] as Role[]
    return (
  ...
  if (moduleKey === 'system-settings') {
    const allowedRoles = moduleRoleMap['system-settings'] as Role[]
    return (
      <RoleGuard allowedRoles={allowedRoles}>
  ...
  if (
    ...
    const allowedRoles = moduleRoleMap[moduleKey] as Role[]
    return (
      <RoleGuard allowedRoles={allowedRoles}>
        <AccountantOperationsModule moduleKey={moduleKey} />
      </RoleGuard>
    )
  }
  ...
  if (
    ...
    return (
      <RoleGuard allowedRoles={allowedRoles}>
        <ExtendedRoleOperationsModule moduleKey={moduleKey} />
      </RoleGuard>
    )
  }
    ...
    <section>
      ...
      >
        <div className="kpi-with-arrow" style={{ gap: '1rem', alignItems: 'flex-start' }}>
          ...
          <div className="quick-actions" style={{ marginBottom: 0, justifyContent: 'flex-end' }}>
            <Button themeColor="primary" onClick={() => navigate(roleDashboardPath(activeRole ?? definition.allowedRoles[0]))}>
              Back to Dashboard
            </Button>
        ...
        {definition.stats.map((stat) => (
          <Card key={stat.label} className="dashboard-card">
            ...
            <CardBody>
              ...
              <p className="card-subtitle" style={{ marginTop: '0.5rem' }}>{stat.detail}</p>
            </CardBody>
          </Card>
        ))}
      </div>
              ...
              {definition.records.map((record) => (
                <li key={record.title}>
                  <div className="kpi-with-arrow">
                    ...
                    <span className="drill-arrow">{record.value}</span>
                  </div>
                  <span className="card-subtitle">{record.detail}</span>
                </li>
              ))}
            </ul>
          </CardBody>
