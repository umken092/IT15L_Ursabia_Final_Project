...
const ApprovedRequestsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchApprovals = async () => {
      try {
        setLoading(true)
        const data = await customerPortalService.getApprovedRequests()
        setApprovals(data)
        setError(null)
      } catch (err) {
        setError('Unable to load approved requests.')
  ...
  if (loading) {
    return <div className="p-4">Loading approved requests...</div>
  }
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold mb-6">Approved Requests</h1>
            ...
            <div key={approval.id} className="bg-white rounded-lg shadow p-6">
              ...
              <div className="flex justify-between items-center text-sm text-gray-500">
                ...
                {approval.approvedDate && (
                  <span>Approved: {new Date(approval.approvedDate).toLocaleDateString()}</span>
                )}
              </div>
            </div>
          ))}
